...
class MaskBase(object):
    ...
    def create_padding_positions_masks(self, nheads):
        ...
        self._i_mask_pp_k = concatenate_to_batch(
            [concatenate_cols([self._i_seq_mask] * l)] * nheads)  # (l, l), batch_size * nheads
    ...
    def create_padding_positions_masks_with_src(self, _i_src_seq_mask, nheads):
        ...
        # key+query mask
        self._i_mask_pp_k = concatenate_to_batch(
            [concatenate_cols([_i_src_seq_mask] * ly)] * nheads)  # (lx, ly), batch_size * nheads
        self._i_mask_pp_q = concatenate_to_batch(
            [concatenate([self._i_seq_mask] * lx)] * nheads)  # (lx,ly), batch_size * nheads
