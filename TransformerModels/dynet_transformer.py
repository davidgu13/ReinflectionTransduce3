...
class EncoderLayer:
    ...
    def __init__(self, model, options, name=''):
        # self attention
        self.self_attention = ParallelMultiHeadAttentionLayer(model, options.num_units, options.nheads, options.use_bias_in_attn, False, name + '.enc-layer.self')
        # feed-forward
        self.feed_forward = FeedForwardLayer(model, options.num_units, options.n_ff_factor, activation=options.ffl_activation, name=name + '.enc-layer')
...
class DecoderLayer:
    ...
    def __init__(self, model, options, name=''):
        # self attention
        self.self_attention = ParallelMultiHeadAttentionLayer(model, options.num_units, options.nheads, options.use_bias_in_attn, True, name + '.dec-layer.self')
        self.src_attention = ParallelMultiHeadAttentionLayer(model, options.num_units, options.nheads, options.use_bias_in_attn, False, name + '.dec-layer.src')
        # feed-forward
        self.feed_forward = FeedForwardLayer(model, options.num_units, options.n_ff_factor, activation=options.ffl_activation, name=name + '.dec-layer')
...
class Embeddings:
    def __init__(self, model, size, options, force_no_seg=False, name=''):
        ...
        if self.use_sinusoidal_encodings:
            ...
            self._positional_enc = model.add_lookup_parameters((options.max_pos_len, options.num_units), name=name + '.embeddings.poslp')
        
        self._seg_enc = model.add_lookup_parameters((options.seg_tok_types, options.num_units), name=name + '.embeddings.seglp') if options.seg_tok_types and not force_no_seg else None
    ...
    def __call__(self, sentences, enc_mask=None, dropout=0.1):
        ...
        else: embeds = self._positional_enc(embeds)
        ...
        if enc_mask is not None:
            src_mask.create_padding_positions_masks_with_src(enc_mask.get_seq_mask(), self.nheads);#note: if we were using the not-parallel attention, we would put here 1
...
class Encoder:
    def __init__(self, model, embed, options):
        ...
        if self.add_pooler_output:
            self._pooler = LinearLayer(model, options.num_units, options.num_units, True, use_he_init=False, name="encoder.pooler")
...
class Generator:
    def __init__(self, model, options):
        ...
        for i in range(options.generator_layers - 1):
            self.layers.append(LinearLayer(model, options.num_units, options.num_units, True, use_he_init=False, name="generator"))
            self.layers.append(options.ffl_activation)
            self.layers.append(NormalizationLayer(model, options.num_units, "generator"))
        # add final layer
        self.layers.append(LinearLayer(model, options.num_units, options.tgt_vocab_size, True, use_he_init=False, name="generator.out"))
    def __call__(self, x):
        out = x
...
class Transformer:
    def __init__(self, options):
        self.model = dy.Model()
        self.encoder = Encoder(self.model, Embeddings(self.model, options.src_vocab_size, options, name='encoder'), options)
        self.has_decoder = options.has_decoder
        if self.has_decoder:
            self.decoder = Decoder(self.model, Embeddings(self.model, options.tgt_vocab_size, options, force_no_seg=True, name='decoder'), options)
...
class BertModel(Transformer):
    def __init__(self, options):
        ...
        self.cls = LinearLayer(self.get_model(), options.num_units, 2, True, use_he_init=False, name='cls.seq-relationship')
...
class TransformerOptions:
    def __init__(self, src_vocab_size, tgt_vocab_size, num_units = 512, nheads = 8, has_decoder = True, use_bias_in_attn = True, n_ff_factor = 4, nlayers = 6, use_label_smoothing = True, label_smoothing_weight = 0.1, ffl_activation=dy.rectify, seg_tok_types=0, fscale_emb=True, add_pooler_output=False, generator_layers=1, norm_residual_layer=False, use_sinusoidal_encodings=True, max_pos_len=-1):
...
if __name__ == '__main__':
    ...
    from transformer_train_decode import transformer_greedy_decode, transformer_beam_decode
