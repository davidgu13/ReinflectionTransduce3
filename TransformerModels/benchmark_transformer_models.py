# Benchmarks vs pytorch, using the same model (+ some pytorch -> dynet converter)
use_cuda = False
if use_cuda:
    import dynet_config
...
pairs = [('bert-base-cased-dynet_model.model',
          dytf.TransformerOptions(28996, 28996, num_units=768, nheads=12, nlayers=12, has_decoder=False,
                                  ffl_activation=gelu, use_bias_in_attn=True, seg_tok_types=2, fscale_emb=False,
                                  add_pooler_output=True, generator_layers=2, norm_residual_layer=True,
                                  use_sinusoidal_encodings=False, max_pos_len=512)),
...
for model_fname, config in pairs:
    ...
    for slice_size in [8, 64, 128, 256]:
        ...
        for batch_size in batch_sizes:
            ...
            seq_pt = (torch.tensor(x, device='cpu').repeat(batch_size, 1) for x in
                      batch_it(tokenized_sequence, slice_size))
