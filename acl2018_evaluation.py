...
def aggregate_results(results):
    ...
    for (config,con,dec),folds in results.items():
        ...
        for foldname,fold in list(folds.items()):
            if 'Q' in options.mode:
                seedaccurracies = list(fold.values())[:1] if list(fold.values()) else []  # pick one
#                SUPPORT_STATISTICS[(config,con,dec,model,align,mode,foldname)] += 1
            else:
                seedaccurracies = []
                for seed_acc in list(fold.values()):
    ...
    for (config,con,dec),folds in results.items():
        ...
        for foldname, fold in folds.items():
            ...
            if foldname == '__STATS__':
                ...
                for seedname,seed in fold.items():
